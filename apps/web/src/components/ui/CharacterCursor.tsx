...
const CharacterCursor: React.FC<CharacterCursorProps> = ({
  ...
  useEffect(() => {
    // Create the canvas imperatively so React never owns it in the DOM tree.
    // If we let React render <canvas> and then move it with appendChild, React
    // loses track of the node and throws removeChild errors on unmount.
    const imperativeCanvas = document.createElement('canvas');
    canvasRef.current = imperativeCanvas;
